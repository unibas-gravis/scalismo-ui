  ...
  class CustomCardLayout extends CardLayout {
    override def preferredLayoutSize(parent: awt.Container): Dimension = {
      parent.getTreeLock synchronized {
        val sizes = activeCards.map(_.getPreferredSize)
    ...
    private[CardPanel] var activeCards: List[AComponent] = Nil
    var minimumWidth = 0
    var minimumHeight = 0
  }
...
class CardPanel extends Panel with LayoutContainer {
  ...
  private var cards: Map[UniqueID, Component] = Map.empty
  ...
  private def show(id: UniqueID): Unit = {
    _current = id
