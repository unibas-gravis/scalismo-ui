...
object SimpleAPI {
  ...
  class ScalismoUI(val app: ScalismoApp) {

    implicit val scene = app.scene
     ...
     */
    def addLandmarksTo(landmarks: IndexedSeq[Landmark[_3D]], sceneObjectName: String) {
     ...
     */
    def show[P: Numeric: ClassTag: TypeTag](image: DiscreteScalarImage[_3D, P], name: String) {
     ...
     */
    def show(pc: Iterable[Point[_3D]], name: String) {
     ...
     */
    def show(m: TriangleMesh, name: String) {
     ...
     */
    def show(sm: StatisticalMeshModel, name: String) {
