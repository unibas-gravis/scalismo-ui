...
trait SimpleAPI {
  ...
  def show[A](a: A, name : String, group : Group)(implicit showInScene: ShowInScene[A]) : showInScene.View = showInScene.showInScene(a, name, group)
  ...
  def filter[V <: ObjectView : FindInScene](pred : V => Boolean, group : Group) : Seq[V] = {
  ...
  def find[V <: ObjectView: FindInScene](pred : V => Boolean, group : Group) : Option[V] =
    filter[V](pred).headOption
