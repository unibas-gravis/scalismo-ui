...
class DiscreteLowRankGpPointTransformation private (val dgp: DiscreteLowRankGaussianProcess[_3D, DiscreteDomain[_3D], Vector[_3D]], gp: LowRankGaussianProcess[_3D, Vector[_3D]], coefficients: DenseVector[Double]) extends LowRankGpPointTransformation(gp, coefficients) {

  protected def this(dgp: DiscreteLowRankGaussianProcess[_3D, DiscreteDomain[_3D], Vector[_3D]],
    coefficients: DenseVector[Double]) = {
    this(dgp, dgp.interpolate(NearestNeighborInterpolator[_3D, Vector[_3D]]()), coefficients)
  }
...
object DiscreteLowRankGpPointTransformation {
  def apply(dgp: DiscreteLowRankGaussianProcess[_3D, DiscreteDomain[_3D], Vector[_3D]]): DiscreteLowRankGpPointTransformation = apply(dgp, DenseVector.zeros[Double](dgp.rank))

  def apply(dgp: DiscreteLowRankGaussianProcess[_3D, DiscreteDomain[_3D], Vector[_3D]], coefficients: DenseVector[Double]): DiscreteLowRankGpPointTransformation = new DiscreteLowRankGpPointTransformation(dgp, coefficients)
}
