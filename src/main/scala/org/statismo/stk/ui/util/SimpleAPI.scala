...
import org.statismo.stk.core.geometry.Landmark
...
object SimpleAPI {
 /**
   ...
   */
  def show(pc: Iterable[Point[_3D]], name: String)(implicit s: Scene) { StaticPointCloud.createFromPeer(pc.toIndexedSeq, None, Some(name)) }
   ...
   */
  def show(m: TriangleMesh, name: String)(implicit s: Scene) { StaticMesh.createFromPeer(m, None, Some(name)) }
   ...
   */
  def show(sm: StatisticalMeshModel, name: String)(implicit s: Scene) { ShapeModel.createFromPeer(sm, 1, Some(name)) }
}